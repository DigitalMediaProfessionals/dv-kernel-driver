...
phys_addr_t tvgen_bar_physical[TVGEN_DEV_NUM];
...
{
    tvgen_bar_physical[idx % TVGEN_DEV_NUM] = addr;
}
...
void tvgen_start(const char* path)
{
    ...
    // write init sequence
    {
        struct list_head *list;
        struct list_init *data;
        list_for_each_prev(list, &list_init_head)
        {
...
{
    volatile u32* mem = (volatile u32*)logical;
...
void tvgen_mem_table(const struct dmp_dv_kbuf* buf, dma_addr_t physical, u64 size)
{
    ...
    tvgen_mem_write(file_memdump, logical + buf->offs, physical, size, "table");
