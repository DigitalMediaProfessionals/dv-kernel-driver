...
#define io_write32(A,B) iowrite32(B,pdc_addr+A); rwLog[l++]=A; rwLog[l++]=B
#define io_read32(A) ioread32(pdc_addr+A)
static void lcdc_set_parameters (pdc_context_t *ctx, void __iomem *pdc_addr)
{
    display_info_t *info;
    ...
    /* total count */
    val  = info->left_margin  + info->xres + info->right_margin + info->hsync_len;
    val2 = info->upper_margin + info->yres + info->lower_margin + info->vsync_len;
    ...
    if(info->reverse) {
      ...
      val2 = info->upper_margin + info->lower_margin + info->vsync_len;
    }
    else {
      val  = info->left_margin;
      val2 = info->upper_margin;
    }
    io_write32(REG_H_ADDR_TIME_START, val);
    io_write32(REG_V_ADDR_TIME_START, val2);
    ...
    io_write32(REG_H_LEFT_BORDER, val);                               
    io_write32(REG_V_TOP_BORDER,  val2);                              
    ...
    if(info->reverse) {
      ...
      io_write32(REG_V_BOTTOM_BORDER, val2-1);      
    }
    else {
      val  = info->left_margin  + info->xres;
      val2 = info->upper_margin + info->yres;
      io_write32(REG_H_BLANK, val);
      io_write32(REG_H_RIGHT_BORDER, val);
      io_write32(REG_V_BLANK, val2);
      io_write32(REG_V_BOTTOM_BORDER, val2);
    }
    ...
    io_write32(REG_H_SYNC, val);
    io_write32(REG_V_SYNC, val2);
    ...
    io_write32(REG_H_INT_TIMING, val);
    io_write32(REG_V_INT_TIMING, val2);

    /* DMA timing -- during 1 clock (for H) */
    val = 1 << 16;
    io_write32(REG_H_DMA_TIMING, val);
    io_write32(REG_V_DMA_TIMING, 10);
    ...
    if(info->reverse) val=0;
    else val = info->left_margin  + info->xres + info->right_margin;
    io_write32(REG_V_INC_H_VALUE, val);

    /* signal polarity */
    val = (info->vsync_pol << 4) | info->hsync_pol;
    io_write32(REG_SIGNAL_POL, val);
    ...
    io_write32(REG_PIC_BORDER_V, (val2 << 16) | val);

    /* FB address, stride */
    io_write32(REG_FB0_ADDR, ctx->open->fb0_addr);
    //io_write32(REG_FB1_ADDR, ctx->open->fb1_addr);   /* unused in new PDC */
    io_write32(REG_FB_SIZE,  ctx->open->fb_stride);
    if (ctx->open->write_back && ctx->ch == 3)
        io_write32(REG_FB_W, ctx->open->fbw_addr);

    /* Mode -- format, burst length, req interval */
    val = ctx->open->fb_format;
    if (val >= LCDC_FORMAT_ARGB8888)  /* need to swap -- set to other register */
        val = LCDC_FORMAT_RGBA8888;

    val2 = 0x00080200; //(8 << 16) | (3 << 8) | (1 << 7) | val;
    io_write32(REG_MODE, val2);
    ...
    if (info->xres_virtual != ctx->open->image_width) {
        ...
        val2 = ((ctx->open->image_width % info->xres_virtual) << 20) /
                                                         info->xres_virtual;
        io_write32(REG_X_SCALE_FACTOR, (val << 20) | val2);
    }
    if (info->yres_virtual != ctx->open->image_height) {
        ...
        val2 = ((ctx->open->image_height % info->yres_virtual) << 20) /
                                                         info->yres_virtual;
        io_write32(REG_Y_SCALE_FACTOR, (val << 20) | val2);
    }
#endif
#if 0
    for(val=0; val<l; val+=2) {
      printk(KERN_INFO "\treg: %04x_%08x\n",rwLog[val],rwLog[val+1]); 
    }
#endif
}
#undef io_write32
...
void pdc_config(void __iomem *pdc_addr, int* dims, unsigned int* fbPA)
{
  ...
  if((width==1024)&&(height==768)&&(pol&8)) dispType=DISP_VESAXGA_RB;
  else if((width==1024)&&(height==768)) dispType=DISP_VESAXGA;
  else if((width==1280)&&(height==720)) dispType=DISP_720p;
  else dispType=DISP_VESAVGA;   // TODO!
  ...
  Lopen.fb_stride    = IMAGE_WIDTH*4;
  Lopen.fb_format    = LCDC_FORMAT_RGBA8888;
...
#if 0
  iowrite32(0, pdc_addr+REG_RAM_ADDR);
