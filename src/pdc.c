...
#define io_write32(A, B)                                                       \
	iowrite32(B, pdc_addr + A);                                            \
	rwLog[l++] = A;                                                        \
	rwLog[l++] = B
...
static void lcdc_set_parameters(pdc_context_t *ctx, void __iomem *pdc_addr)
{
	...
	/* NOTE2 : the timing parameter of "display_info_t" is correponded to
		   Linux FB. refer to "Documentation/fb/framebuffer.txt" for
	   detail.
		   the basic image is as following:
		       H : backporch(left margin) -> L_border=0  -> active image
	   -> R_border=0 -> frontportch(right margin) -> hsync
		       V : backporch(upper margin)-> T_border=0  -> active image
