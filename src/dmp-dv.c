...
MODULE_PARM_DESC(memGuess,
		 "use unallocated mem setup based on actual previous alloc");
...
static irqreturn_t handleInt_p(int irq, void *dev_id)
{
	...
	// if(sts & 0x8000) printk(KERN_INFO "==> VINT: WARNING: UINT! sts =
	// 0x%08x\n",sts);
	...
	// note: we cannot use KINT with the below reg consumption polling
	// method since
	// consumption itself causes KINT, so we just end up waiting for, and
	// processing
	...
	if (subdev->int_status == 1) { // user waiting for int/swap
		...
		iowrite32(memSec[subdev->bank ? 2 : 0],
			  REG_IO_ADDR(subdev, PDC_REG_FBADDR));
		// wait for FB addr update consume:
		rsvStat = 1;
		while (rsvStat & 1)
			rsvStat = ioread32(REG_IO_ADDR(subdev, 0x94));
...
static unsigned int waitInt_p(struct dmp_dev *subdev)
{
	...
	r = wait_event_interruptible(subdev->int_status_wait,
				     (subdev->int_status & 2));
	if (!r) {
			     ...
			     struct drm_dev *drm_dev)
{
	...
	*sizeO = 0;
	...
	if (dma_set_mask_and_coherent(drm_dev->dev, DMA_BIT_MASK(32))) {
		printk(KERN_INFO
		       "==>DMP_drm: no suitable DMA available(?!?)\n");
		return 0;
	}
	...
	*sizeO = size;
	if (vrb == 1) {
		printk(KERN_INFO
		       "==>DMP_drm: cohDMA alloc @ PA 0x%08x VA 0x%08x (0x%08x)\n",
		       (unsigned int)HND, (unsigned int)VA,
		       (unsigned int)(__va((void *)HND)));
	}
	...
	if (vrb == 1) {
		printk(KERN_INFO
		       "==>DMP_drm: DMA alloc @ PA 0x%08x VA 0x%08x\n",
		       (unsigned int)(__pa(VA)), VA);
	}
	if (SZ != size)
		printk(KERN_INFO
		       "==>DMP_drm: warning! kmalloc'd size<requested\n");
	*sizeO = SZ;
	return __pa(VA);
...
static int memSetup(struct drm_dev *drm_dev)
{
	...
	if (memGuess != 0) {
		...
		printk(KERN_INFO
		       "==>INFOm0: FB0 allocated: addr=0x%08x size=0x%08x bytes\n",
		       memSec[0], memSec[1]);
		printk(KERN_INFO
		       "==>INFOm1: FB1 allocated: addr=0x%08x size=0x%08x bytes\n",
		       memSec[2], memSec[3]);
		printk(KERN_INFO
		       "==>INFOm2: Workmem %dMB allocated in 1 sector: addr=0x%08x\n",
		       memSec[5] / (1024 * 1024), memSec[4]);
		return 0;
	} else {
		if (fbSize > KMALLOC_MAX_SIZE) {
			printk(KERN_INFO
			       "==>ERROR! framebuf size must be < KMALLOC_MAX_SIZE for dynamic alloc\n");
			return 1;
		}
		...
		if (allocT < allocTGT) {
			printk(KERN_INFO
			       "==>ERROR! could note allocate work area target\n");
			return 1;
		} else {
			// sort in descending size order:
			int secOrd[MEM_SEC_N];
			unsigned int memSecCP[MEM_SEC_N * 2];
			for (k = 0; k < MEM_SEC_N * 2; k++)
				memSecCP[k] = memSec[k + 4];
			...
			if (k != j) {
				printk(KERN_INFO
				       "==>ERROR! mem sector sorting problem %d!=%d\n",
				       k, j);
				return 1;
			}
			for (k = 0; k < j; k++) {
			...
			printk(KERN_INFO
			       ...
			       memSec[2], memSec[3]);
			printk(KERN_INFO
			       "==>INFOm2: Workmem %dMB allocated in %d sectors:\n",
			       workmemMB, j);
			for (k = 0; k < j; k++) {
				printk(KERN_INFO
				       "==>  %02d: addr=0x%08x size=0x%08x bytes\n",
				       k, memSec[2 * (2 + k)],
				       memSec[2 * (2 + k) + 1]);
			}
			printk(KERN_INFO "\n");
...
static long drm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case CNV_MEMSEC: {
		...
		for (k = 0; (ret != -EFAULT) && (k < MEM_SEC_N); k++) {
			ret = put_user(memSec[(2 + k) * 2],
				       (unsigned int __user *)usrA);
			if (ret)
				ret = -EFAULT;
			else {
				ret = put_user(
					memSec[(2 + k) * 2 + 1],
					(unsigned int __user *)(usrA + 4));
				if (ret)
					ret = -EFAULT;
			}
			if (memSec[(2 + k) * 2] == 0)
...
static int drm_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	err = io_remap_pfn_range(vma, vma->vm_start, pfn, map_size,
				 vma->vm_page_prot);
	if (err) {
		return -EAGAIN;
		goto fail_ioremap;
...
int drm_register_chrdev(struct drm_dev *drm_dev)
{
	...
	for (i = 0; i < DRM_NUM_SUBDEV; i++) {
		...
		if (i == 0) {
			if (!err)
				err = request_irq(rIRQ, handleInt_c,
						  IRQF_SHARED, DRM_DEV_NAME,
						  &(drm_dev->subdev[i]));
		} else if (i == 1) {
			if (!err)
				err = request_irq(rIRQ, handleInt_p,
						  IRQF_SHARED, DRM_DEV_NAME,
						  &(drm_dev->subdev[i]));
		} else {
			if (!err)
				err = request_irq(rIRQ, handleInt_f,
						  IRQF_SHARED, DRM_DEV_NAME,
						  &(drm_dev->subdev[i]));
		}
	...
	pdc_config((void __iomem *)drm_dev->subdev[1].bar_logical, pdcDims,
		   fbPA);
	if (pStart)
		pdc_start((void __iomem *)drm_dev->subdev[1].bar_logical);
